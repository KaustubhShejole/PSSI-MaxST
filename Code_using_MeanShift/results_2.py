...
segmented_image = cv2.imread(mask_img_path2+str(iteration_count_2) + scribbles_from +
...
our_mask = cv2.imread(mask_img_path2+str(iteration_count_2) + scribbles_from +
                      '.png')
...
plt.imshow(result)
plt.imsave(only_segmentation_path + '/2/'+image_num + scribbles_from + '.png',
           result)
plt.axis('off')
# plt.show()
plt.close()
