...
segmented_image = cv2.imread(
    mask_img_path + '_1_1_'+str(iteration_count_1)+'.png', cv2.IMREAD_GRAYSCALE)  # Convert to grayscale
ground_truth = cv2.imread(ground_truth_path,
                          cv2.IMREAD_GRAYSCALE)  # Convert to grayscale
our_mask = cv2.imread(mask_img_path + '_1_1_'+str(iteration_count_1)+'.png')
...
if not os.path.exists(path_to_add + 'results/text_results/'+base_name):
    # Create the folder
    os.makedirs(path_to_add + 'results/text_results/'+base_name)
