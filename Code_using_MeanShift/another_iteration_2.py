...
file_to_show_path = seg_img_path2 + str(iteration_count_2) + '.png'
...
plt.imshow(image)
plt.imsave(scribbled_img_path2 +
           str(iteration_count_2 + 1)+'.png', image)
plt.axis('off')
plt.show()
...
for each_superpixel in new_background_clicked_points:
    if not directed_graph1.has_edge('background', each_superpixel):
        directed_graph1.add_edge('background', each_superpixel, weight=np.inf)

for each_superpixel in new_foreground_clicked_points:
    if not directed_graph1.has_edge(each_superpixel, 'foreground'):
        directed_graph1.add_edge(each_superpixel, 'foreground', weight=np.inf)
...
mask1 = image_mask_max_flow(
    np.copy(original_image), foreground_set, background_set, labels_slic)
plt.imshow(mask1)
plt.imsave(mask_img_path2 + str(iteration_count_2+1) + '.png',
           mask1)
plt.axis('off')
plt.show()
