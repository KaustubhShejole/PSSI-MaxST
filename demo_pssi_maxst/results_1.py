...
our_mask = cv2.imread('results/mask_' + str(image_num) + '_' +
...
output_dir = os.path.join('text_results')
...
with open(os.path.join(output_dir, unique_filename), 'w') as file:
    ...
    file.write(f"Recall (TPR): {recall}\n")
    ...
    file.write(f"Error Rate: {error_rate}\n")

    # Timing Information
    file.write(f"\nTotal Time taken (Iteration 1) in seconds: {elapsed_time}\n")
    for idx, elapsed_time4 in enumerate(elapsed_time_4_list, start=2):
        file.write(f"Iteration {idx} time: {elapsed_time4}\n")
